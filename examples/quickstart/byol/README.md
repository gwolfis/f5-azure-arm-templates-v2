...
- **Access**: This template creates an Azure Managed User Identity, KeyVault, and secret used to set the admin password on the BIG-IP instance.
...
F5 has provided the following F5 BIG-IP Runtime Init configurations and example declarations for the supported Automation Toolchain components in the examples/quickstart/bigip-configurations folder:

- quickstart-do-1nic-byol.json: The Declarative Onboarding declaration configures L2/L3 networking and system settings on a BYOL 1NIC BIG-IP. See the [DO documentation](https://clouddocs.f5.com/products/extensions/f5-declarative-onboarding/) for details on how to use DO. Additional example declarations are provided for PAYG and BIG-IQ licensing.
- quickstart-do-2nic-byol.json: The Declarative Onboarding declaration configures L2/L3 networking and system settings on a BYOL 2NIC BIG-IP. Additional example declarations are provided for PAYG and BIG-IQ licensing.
- quickstart-do-3nic-byol.json: The Declarative Onboarding declaration configures L2/L3 networking and system settings on a BYOL 3NIC BIG-IP. Additional example declarations are provided for PAYG and BIG-IQ licensing.
- quickstart-as3.json: The Application Services declaration configures L4-L7 application services on BIG-IP, including service discovery. See the [AS3 documentation](https://clouddocs.f5.com/products/extensions/f5-appsvcs-extension/) for details on how to use AS3.
- quickstart-ts.json: The Telemetry Streaming declaration configures BIG-IP to declaratively aggregate, normalize, and forward statistics and events to a consumer application. See the [TS documentation](https://clouddocs.f5.com/products/extensions/f5-telemetry-streaming/) for details on how to use TS.
...
Here is an example F5 BIG-IP Runtime Init configuration that uses the previously referenced Automation Toolchain declarations to deploy a BYOL 3NIC BIG-IP:
...
| appContainer | No | The name of a container to download and install which is used for the example application server(s). If this value is left blank, the application module template is not deployed. |
...
[![Deploy to Azure](http://azuredeploy.net/deploybutton.png)](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2FF5Networks%2Ff5-azure-arm-templates-v2%2Fv1.0.0.0%2Fexamples%2Fquickstart%2Fbyol%2Fazuredeploy.json)
...
- Obtain the IP address of the BIG-IP instance(s):
  ...
  - **Azure CLI**: 
    ...
    - Private IPs: ```az vm show --vmss-name **instance name** -g **your resource group name** -d -o json --query privateIps```
