...
| bigIqLicensePoolName | No | The BIG-IQ license pool used during BIG-IP licensing via BIG-IQ. |
| bigIqUtilityKey | No | The BIG-IQ utility license key used during BIG-IP licensing via BIG-IQ. This value should match the BIG-IQ utility key specified in the F5 Declarative Onboarding declaration passed to the bigIpRuntimeInitConfig template parameter. ***Important***: This is only required when revoking a license from an ELA/subscription (utility) pool on the BIG-IQ; if you're not using this pool type, leave the default of **Default**. |
